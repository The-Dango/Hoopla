...
const Engine = (() => {
  ...
  function solve(p, limit = 2, nodeLimit = 3e6) {
    ...
    // Givens are hoops placed for the player, so every solution must hold them.
    // Without this, pinning a given to rule out a second solution changed nothing.
    const given = new Uint8Array(W * H); for (const c of p.givens || []) given[c] = 1;
    // quick infeasibility
  ...
  function regionConnectedWithout(W, H, reg, g, removed) { // removed: one cell, or a Set of them
    const gone = typeof removed === 'number' ? new Set([removed]) : removed;
    const cells = []; for (let c = 0; c < W * H; c++) if (reg[c] === g && !gone.has(c)) cells.push(c);
  ...
  // Most hoops a board may start with, by size: small boards none or one, medium up to
  // two, large up to four. Shared by the generator's pins and the builder's, so the
  // total a player is handed never goes past it.
  function givenCap(mask) { const n = mask.filter(Boolean).length; return n <= 64 ? 1 : n <= 90 ? 2 : 4; }
  ...
  const WORK_CAP = 12.5e6, BATCH = 64, STRIP_MAX = 4;
  function generate({ shape = 'square', k = 1, size = 0, holes = false, regions = true, seed = Date.now(), workCap = WORK_CAP, meter = null } = {}) {
    ...
    for (;;) {
      ...
      let good = false; const cap = givenCap(mask);
      for (let rep = 0; rep < 80; rep++) {
        ...
        // Without regions, ask for a batch of solutions so the next given can be chosen well.
        const res = solve(p, regions ? 2 : BATCH, Math.min(4e6, workCap - work + 1)); work += res.nodes; if (meter) meter.work += res.nodes;
        ...
        if (!alt) break;
        if (!regions) { // no regions to reshape, so pin down a star: the one that the most
          // other solutions lack, so as few hoops as possible are handed to the player
          if (p.givens.length >= cap) break;
          const seen = new Map(); for (const s of res.sols) for (const c of s) seen.set(c, (seen.get(c) || 0) + 1);
          let pick = -1, fewest = Infinity;
          for (const c of shuffle(p.solution.filter(c => !p.givens.includes(c)), r)) {
            const n = seen.get(c) || 0; if (n < fewest) { fewest = n; pick = c; } }
          if (pick < 0) break;
          p.givens.push(pick); repairsTotal++; continue;
        }
        ...
        // No neighbour colour touches the stray hoop: carry a strip of its colour, from
        // the hoop to the nearest square that does touch another colour, over into that
        // colour. The strip holds no real hoop, so the true solution is untouched.
        for (const c of cand) { if (moved) break; const g = region[c];
          const prev = new Map([[c, -1]]), q = [c]; let end = -1, h = -1;
          for (let qi = 0; qi < q.length && end < 0; qi++) { const u = q[qi], x = u % W, y = (u / W) | 0;
            for (const [dx, dy] of ADJ4) { const nx = x + dx, ny = y + dy; if (nx < 0 || ny < 0 || nx >= W || ny >= H) continue;
              const v = ny * W + nx, o = region[v]; if (o < 0) continue;
              if (o !== g) { end = u; h = o; break; }
              if (!prev.has(v) && !truth.has(v) && !p.givens.includes(v)) { prev.set(v, u); q.push(v); } } }
          if (end < 0) continue;
          const strip = new Set(); for (let u = end; u >= 0; u = prev.get(u)) strip.add(u);
          if (strip.size > STRIP_MAX || !regionConnectedWithout(W, H, region, g, strip)) continue;
          for (const u of strip) region[u] = h; moved = true; repairsTotal++; }
        if (!moved) {
          // Still stuck: rather than throw away the whole board, place one of the real
          // hoops for the player, which the other solution lacks. Never past givenCap,
          // and the pass below takes back any that turn out not to be needed.
          if (p.givens.length >= cap) break;
          const pin = shuffle(p.solution.filter(c => !alt.includes(c) && !p.givens.includes(c)), r)[0];
          if (pin === undefined) break;
          p.givens.push(pin); repairsTotal++;
        }
      }
      if (!good) continue;
      // Pinned hoops got the board to one solution quickly, but later repairs often make
      // some of them unnecessary. Take each away in turn and keep it off if the puzzle
      // is still unique without it: the player should be handed as little as possible.
      for (const c of shuffle(p.givens.slice(), r)) {
        const keep = p.givens; p.givens = keep.filter(g => g !== c);
        const res = solve(p, 2, 4e6); work += res.nodes; if (meter) meter.work += res.nodes;
        if (res.aborted || res.sols.length !== 1) p.givens = keep;
      }
  ...
  return { generate, solve, givenCap, SIZES, Shapes };
})();
